...
auth_config = AuthXConfig(
    JWT_SECRET_KEY=settings.JWT_SECRET_KEY,  # вынеси в .env
...
def require_admin(
    user: dict = Depends(require_user)  # переиспользуем require_user
):
...
def refresh_token(
    request: Request,
    response: Response,
    token: str = Depends(auth.refresh_token_required),
):
...
@router.get("/me")
def get_me(
    token: str = Depends(auth.access_token_required), db: Session = Depends(get_db)
):
    user_id = token.sub
