        ...
        <div class="bg-base-200 rounded-xl text-center mb-4 py-4 flex justify-around flex-wrap  md:hidden">
          ...
          <div v-else>
            <div v-if="!isFavorite" class="tooltip tooltip-left" data-tip="Добавить в избранное">
              <button @click="toggleFavorite" v-if="authUser" class="btn btn-outline p-2 w-10 h-10 md:w-full md:h-auto">
            ...
            <div v-if="isFavorite" class="tooltip tooltip-left" data-tip="Удалить из избранного">
              <button @click="toggleFavorite" v-if="authUser"
    ...
    <dialog id="imageModal" class="modal" :open="isModalOpen" @click.self="closeModal">
      ...
      <div
        ...
        <div class="flex-1 overflow-hidden flex justify-center items-center relative">
          ...
          }" alt="Фото автомобиля" draggable="false" @touchstart="startTouch" @touchmove="moveTouch"
        ...
        <div class="sticky bottom-0 bg-base-100 bg-opacity-90 p-2 flex justify-center gap-4 z-10">
          ...
          <button @click.stop="nextImage" :disabled="currentIndex === car.images.length - 1" class="btn btn-circle">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
...
async function checkFavorite() {
  ...
  try {
    const favRes = await api.get(`/favorites/check/${authUser.value.uuid}/${car.uuid}`)
  ...
  try {
    const ownRes = await api.get(`/cars/check_ownership/${car.uuid}/${authUser.value.uuid}`)
...
function openChat(carUuid, otherUserUuid) {
  router.push({
    name: 'Chat',
    params: { carUuid: carUuid, otherUserUuid: otherUserUuid },
  });
}
